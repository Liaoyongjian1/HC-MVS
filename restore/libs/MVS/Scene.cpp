...
bool Scene::LoadInterface(const String & fileName)
{
	...
	for (const Interface::Image& image: obj.images) {
		...
		// Prepare label image
		imageData.maskName = Util::getFilePath(image.name) + PATH_SEPARATOR +"seman" + PATH_SEPARATOR + Util::getFileName(image.name) + ".quad.png";
		...
		imageData.coloredMaskName = Util::getFilePath(image.name) + PATH_SEPARATOR + Util::getFileName(image.name) + ".quad.JPG";
		//imageData.maskName = Util::getLastDir(Util::getFilePath(imageData.name)) + Util::getFileName(imageData.name) + "_l.png";
