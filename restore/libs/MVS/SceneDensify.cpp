...
bool DepthMapsData::InitViews(DepthData& depthData, IIndex idxNeighbor, IIndex numNeighbors)
{
	...
	// std::cout<<"领域="<<depthData.neighbors.GetSize()<<std::endl;
	...
	cv::resize(depthData.images[1].pImageData->image, depthData.images[1].pImageData->image, imageRef.pImageData->image.size(), 0, 0, cv::INTER_NEAREST);
	// cv::imwrite("imageRef.png",imageRef.pImageData->image);
...
bool DepthMapsData::InitDepthMap(DepthData& depthData)
{
	...
	// DepthData& depthData(data.depthMaps.arrDepthData[idx]);
			LoadDepthMap(ComposeReadDepthFilePath(depthData.GetView().GetID(), "dmap"), depthData.resize_depthMap) ;
			LoadNormalMap(ComposeReadNormalFilePath(depthData.GetView().GetID(), "dmap"), depthData.resize_normalMap) ;
			...
			cv::resize(depthData.resize_depthMap, depthData.nresize_depthMap, depthData.depthMap.size(), 0 , 0 ,cv::INTER_AREA);
			cv::resize(depthData.resize_normalMap, depthData.nresize_normalMap, depthData.depthMap.size(), 0 , 0 ,cv::INTER_AREA);

			for(int i = 0; i < depthData.nresize_depthMap.rows; i++){
				for(int j = 0; j < depthData.nresize_depthMap.cols; j++){
					float value = depthData.nresize_depthMap(i,j);
...
bool DepthMapsData::GenerateDepthPrior(DepthData& depthData, DepthEstimator::MapRefArr& coords)
{
	...
	if (depthData.labels.Load((*depthData.GetView().pImageData).maskName)) 
	{
		...
		depthData.images.First().depthMapPrior = DepthMap(depthData.GetView().image.size());
		DepthMap& meanshiftpriors_depthMap = depthData.images.First().depthMapPrior;
		depthData.images.First().normalMapPrior = NormalMap(depthData.GetView().image.size());
		NormalMap& meanshiftpriors_normalMap = depthData.images.First().normalMapPrior;
		...
		if (!depthData.labels.empty())
		{
			...
			{
				const ImageRef& coord = coords[idx];   
				meanshiftpriors_depthMap(coord) = 0;                   
				meanshiftpriors_normalMap(coord) = Normal::ZERO;	   
			...
			while (++idx < regionPixels.GetSize()) 
			{
				...
				if (planeId >= 0)
				{
					...
					meanshiftpriors_depthMap(coord) = depthData.images.First().camera.TransformPointW2I3(intersection).z;
					const Normal planeNormal(planes[planeId].first.m_vN.x(), planes[planeId].first.m_vN.y(), planes[planeId].first.m_vN.z()); // Normal in object space	
					const Normal N(depthData.images.First().camera.R * Cast<REAL>(planeNormal)); // Convert back in image space

					meanshiftpriors_normalMap(coord) = N;
				}				
			}
			...
			SaveDepthMap(ComposeMeanshiftDepthPriorsPath(depthData.GetView().GetID(), "dmap"), meanshiftpriors_depthMap);
			SaveNormalMap(ComposeMeanshiftNormalPriorsPath(depthData.GetView().GetID(), "dmap"), meanshiftpriors_normalMap);
			...
			ExportDepthMap(ComposeDepthFilePath(depthData.GetView().GetID(), "prior.png"), meanshiftpriors_depthMap);
			ExportNormalMap(ComposeNormalFilePath(depthData.GetView().GetID(), "prior.normal.png"), meanshiftpriors_normalMap);
...
bool DepthMapsData::GapInterpolation( DepthData& depthData)
{
	...
	ConfidenceMap& confMap = depthData.confMap;
	// DepthData::ViewData& imageRef = depthData.images.First();
	// const uint32_t idxImage(imageRef.pImageData->ID);
	// DepthData& depthData(arrDepthData[idxImage]);
	...
	for (int v=0; v<size.y; ++v) {
		...
		for (int u=0; u<size.x; ++u) {
			// get depth of this location
			const Depth& depth_fuse = depthMap_fuse(v,u);
			...
			if (depth_fuse <= 0) {
				...
				float texture_ratio = texture_diff / n*texture0;
				float depth_ratio = depth_diff / n*depth;
				if(texture_ratio<0.01){
					if(depth_ratio<0.01){
						const float c(confMap.empty() ? 0.f : MINF(confMap(x1_temax), confMap(x0)));
						depthMap_fuse(x0) = depthMap(x0);
						float Distance = SQRT(SQUARE(x0.x-x1_temax.x)+SQUARE(x0.y-x1_temax.y));
						Point2f dir1, dir2;
						Normal2Dir(normalMap(x0), dir1);
						Normal2Dir(normalMap(x1_temax), dir2);
						...
						float diff = ((depthData.graMap(x1_temin)-texture0)/(Distance));
						Point2f dir1, dir2;
						Normal2Dir(normalMap(x0), dir1);
						Normal2Dir(normalMap(x1_temin), dir2);
						const Point2f dirDiff((dir2-dir1)/(Distance));
						depthMap_fuse(x0) = (depth+=diff);
						dir1 += dirDiff;
						Dir2Normal(dir1, normalMap_fuse(x0));
						if (!confMap.empty()) confMap(x0) = c;
					}
					...
					if(depth_ratiomin<0.01){
						...
						Point2f dir1, dir2;
						Normal2Dir(normalMap(x0), dir1);
						Normal2Dir(normalMap(x1_demin), dir2);
						const Point2f dirDiff((dir2-dir1)/(Distance));
						dir1 += dirDiff;
						Dir2Normal(dir1, normalMap_fuse(x0));
						if (!confMap.empty()) confMap(x0) = c;
						...
						const float c(confMap.empty() ? 0.f : MINF(confMap(x1_demax), confMap(x0)));
						float Distance = SQRT(SQUARE(x0.x-x1_demax.x)+SQUARE(x0.y-x1_demax.y));
						float diff = ((depthData.graMap(x1_demax)-texture0)/(Distance));
						Point2f dir1, dir2;
						Normal2Dir(normalMap(x0), dir1);
	...
	ExportDepthMapByJetColormap(ComposeDepthFileSegmentPath(depthData.GetView().GetID(), "png"), depthMap_fuse); 
	// ExportNormalMap(ComposeDepthFileSegmentPath(depthData.GetView().GetID(), "png"), normalMap);
	// for (int v=0; v<size.y; ++v) {
	// 	for (int u=0; u<size.x; ++u) {	
	// 		ImageRef x0(u,v);
...
{	const DepthData::ViewData& image(depthData.images.First());
	...
	for(int i=0 ; i<depthData.labelsNum ;i++)
	{
		...
		for(int j=1 ; j<depthData.segmentLab[i].size() ;j++)
		{
			Point2i x = depthData.segmentLab[i][j] ; 
			Depth d = depthData.depthMap(x) ; 
			
			//std::cout<<"n2.4"<<std::endl ;
			if( d > 0 )
			{ //depthArea.push_back(d); 
			// std::cout<<"n2.5"<<std::endl ;
			// std::cout<<"d  is   "<<d<<std::endl ;

			  //data_depth.at<float>(data_depth_num, 0) = d;
              ++num_n0;
			//std::cout<<"n2.6"<<std::endl ;
			}

		}
		//std::cout<<"num_n0    is   "<<num_n0<<std::endl;
		if(num_n0 > 0)
	 {
			float rate_vote = num_n0/(depthData.segmentLab[i].size() - 1);   
		    cv::Mat data_depth(1, num_n0, CV_32FC1);   
		    int data_num(0);
			for(int j=1 ; j<depthData.segmentLab[i].size() ;j++)
		{
			Point2i x = depthData.segmentLab[i][j] ; 
			...
			// std::cout<<"n2.5"<<std::endl ;
			// std::cout<<"d  is   "<<d<<std::endl ;
                 data_depth.at<float>(0, data_num) = d;
			  //data_depth.at<float>(data_depth_num, 0) = d;
              ++data_num;
			//std::cout<<"n2.6"<<std::endl ;
			}
			...
			// if(depthArea.size() != 0) 
			// {	repre.depth = Ransac(depthArea);	}
			data_depth = data_depth.t();
		std::vector<int> centerIndex;
		cv::Mat classes = MaxMinDisFun(data_depth, 0.5, centerIndex);
		int a_find[centerIndex.size()+1];   
		for(int j = 0; j<centerIndex.size()+1; ++j)
						a_find[j] = 0;

	    for (int i = 0; i < classes.rows; ++i)
		{  
			for(int j = 1; j<centerIndex.size()+1; ++j)
				if(classes.at<int>(i,0) == j)
						++a_find[j];
		}
		...
		int max_find(0);        
		for(int j = 0; j<centerIndex.size()+1; ++j)
			if(a_find[j] > max_a_find)
			{
				max_a_find = a_find[j];
				max_find = j;
			}

		float rate_support = max_a_find/num_n0; 
		repre.confidence = 0.5*rate_vote + 0.5*rate_support;    
		float sum_repre(0);
		float num_repre(0);

	 for (int i = 0; i < classes.rows; ++i)
	   if(classes.at<int>(i,0) == max_find)
	       {
			    sum_repre += data_depth.at<float>(i,0);
				++num_repre;
		   }

		repre.depth = sum_repre/num_repre;         
       // std::cout<<"repre.depth   is    "<<repre.depth<<std::endl ;
		...
		for(int j=1 ; j<depthData.segmentLab[i].size() ;j++)
		{
			...
			Depth dis_cur = d - repre.depth ;
			if(dis_cur<0)
			dis_cur = (-1) * dis_cur ;

			if(dis_cur<dis_depth)
			{
				dis_depth = dis_cur ;
				repre.normal = depthData.normalMap(x) ; 
				repre.x_rep = x;
			}
		}
		// std::cout<<" depth   is        "<<repre.depth<<std::endl ;
		// std::cout<<" normal   is     "<<repre.normal<<std::endl ;
		//std::cout<<" before   nSPconf  conf   is   "<<depthData.superRepre[i].confidence<<std::endl ;
		#if 0
		double nSPconf = estimator.ScoreSuperPixelConfidence(i, repre.depth, repre.normal);    
		//std::cout<<" nSPconf  is     "<<nSPconf<<std::endl ;
		if(nSPconf > depthData.superRepre[i].confidence)              
               {
                     depthData.superRepre[i].depth = repre.depth;
					 depthData.superRepre[i].normal = repre.normal;
					 depthData.superRepre[i].x_rep = repre.x_rep;
					 depthData.superRepre[i].confidence = nSPconf;		 
			   }
		#endif
		             depthData.superRepre[i].depth = repre.depth;
					 depthData.superRepre[i].normal = repre.normal;
					 depthData.superRepre[i].x_rep = repre.x_rep;
					 depthData.superRepre[i].confidence = repre.confidence;		 
	}
		// else
		// 	 {
		// 	 //std::cout<<"enter   0  "<<std::endl;
		// 	       depthData.superRepre[i].depth = 0;
		// 		   depthData.superRepre[i].normal = Normal::ZERO;
		// 		   depthData.superRepre[i].confidence = 0;
		// 		   depthData.superRepre[i].x_rep = Point2(-1,-1);
		// 	 }
			
			
			
			//std::cout<<"confs    is      "<<repre.confidence<<std::endl;
			//std::cout<<"i    is    "<<i<<std::endl;
			//std::cout<<" confidence  is     "<<depthData.superRepre[i].confidence<<std::endl ;	   
		}
